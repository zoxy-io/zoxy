...
| `src/http/h2.zig` | sans-io HTTP/2 server connection engine (Phase 5, slice 3): `Connection.drive` consumes one frame per call, stages control frames, surfaces one `Event` (request/trailers/data/reset/goaway/fatal); fixed stream slots (exhaustion → REFUSED_STREAM after the block still decodes for HPACK sync), CONTINUATION assembly with flood bound, dual-level flow accounting with half-window WINDOW_UPDATE batching; response emission + request semantics are slice 4 |
