...
pub fn TlsRelay(comptime IoType: type) type {
    ...
    return struct {
        ...
        fn onClientRecv(conn: *ConnType, result: Io.RecvError!u32) void {
            ...
            conn.directions[0].owe(@intCast(decrypted.len));
            armUpstreamSend(server, conn);
        }
        ...
        fn armUpstreamSend(
            ...
            const state = &conn.directions[0];
            // The caller establishes the debt with `owe`; a short write
        ...
        fn onUpstreamSend(conn: *ConnType, result: Io.SendError!u32) void {
            ...
            assert(sent >= 1);
            conn.directions[0].credit(sent);
            if (conn.directions[0].owed() > 0) { // Short write: send the rest.
                armUpstreamSend(server, conn);
                return;
            }
            armClientRecv(server, conn);
        ...
        fn armClientSend(server: *ServerType, conn: *ConnType) void {
            assert(conn.state == .relaying);
            ...
            if (outbound.len == 0) {
                // Everything staged has gone out. Whether that was the last
                // of it is the engine's fact, not a flag ridden across the
                // send: close_notify staged means this direction is done.
                if (conn.tls.?.closeStaged()) {
                    finishDirection(server, conn, 1);
                } else {
                    armUpstreamRecv(server, conn);
                }
            ...
            // This direction sends straight from the outbox, which carries
            // its own cursor, so the direction owes no framed debt of its
            // own (§6) — nothing here to `owe` or `credit`.
            conn.directions[1].phase = .sending;
            conn.arm(&conn.op_data_upstream_to_client, "data_upstream_to_client");
            server.io.send(
                conn.client_socket,
