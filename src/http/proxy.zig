...
pub fn Proxy(comptime IoType: type) type {
    ...
    return struct {
        ...
        // -- client-directed writes (§4) --
        //
        // Every response-side send writes an already-materialized
        // plaintext buffer under its own cursor. Under TLS the wire
        // carries ciphertext instead, which does not map onto those
        // bytes one-for-one, so these two keep the two cursors apart:
        // the engine outbox tracks the wire, and the caller's cursor
        // moves only once a chunk is fully out. Each caller's plaintext
        // fits one record (the head buffer bounds all of them, and it is
        // smaller than `max_plaintext_bytes`), so one `sendApp` per chunk
        // is enough.
        ...
        fn clientWriteSlice(conn: *ConnType, plaintext: []const u8) ?[]const u8 {
            ...
            if (engine.outbound().len == 0) {
                // Every caller writes out of the head buffer or the
                // upstream head, both smaller than one record, so a chunk
                // never needs splitting across records.
                assert(plaintext.len <= TlsEngine.max_plaintext_bytes);
                if (engine.outboundRoom() < TlsEngine.max_record_bytes) {
                    // The same staging-room rung the L4 relay sheds on
                    // (§8); counted here too, so the ladder is observable
                    // from either path.
        ...
        /// Credit `sent` wire bytes, returning how many *plaintext* bytes
        /// are now complete: `sent` itself when plain, and under TLS all
        /// of `plaintext_len` once the outbox drains but zero before —
        /// partial ciphertext progress says nothing about plaintext.
        ...
        fn onResponseHeadSent(conn: *ConnType, result: Io.SendError!u32) void {
            ...
            assert(sent >= 1);
            const remaining = conn.l7.rendered_response_len - conn.l7.response_head_sent;
            conn.l7.response_head_sent += clientWriteCredit(conn, sent, remaining);
        ...
        fn onResponseExcessSent(conn: *ConnType, result: Io.SendError!u32) void {
            ...
            assert(sent >= 1);
            const direction = &conn.directions[1];
            direction.sent_len += clientWriteCredit(
                conn,
                sent,
                direction.transfer_len - direction.sent_len,
            );
            assert(direction.sent_len <= direction.transfer_len);
            if (direction.sent_len < direction.transfer_len) {
                armResponseExcessSend(server, conn);
        ...
        fn armResponseSend(server: *ServerType, conn: *ConnType) void {
            ...
            assert(conn.response_pending.len >= 1);
            const wire = clientWriteSlice(conn, conn.response_pending) orelse {
                server.beginTeardown(conn);
                return;
            };
            conn.arm(&conn.op_data_upstream_to_client, "data_upstream_to_client");
        ...
        fn onResponseSent(conn: *ConnType, result: Io.SendError!u32) void {
            ...
            assert(sent >= 1);
            const done = clientWriteCredit(
                conn,
                sent,
                @intCast(conn.response_pending.len),
            );
            assert(done <= conn.response_pending.len);
            conn.response_pending = conn.response_pending[done..];
